...
считается по тексту, обрезки многоточием быть не должно. Фиксированные 172 px Handy рассчитаны
...
1. ✅ **Vosk и фильтр «Только отечественные»** — решено 2026-09-08: происхождение «уточняется»,
   в фильтр отечественных карточка **не попадает** до подтверждения ресёрчером; в профиле
   `domestic_only=true` Vosk не показывается.
...
5. ⏳ **Цифры качества и скорости для четырёх добавленных моделей** (Vosk ru 0.54, Whisper small,
   GigaAM ML Large, NeMo FastConformer) в макете — правдоподобные заглушки: в PRD §7.2 для них
   есть только диск/ОЗУ/язык/лицензия. Нужен замер или источник от Researcher.
