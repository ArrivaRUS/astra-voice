...
## 8.1 Что вынести на приёмку полного макета (⛔ G3)

1. **Vosk и фильтр «Только отечественные».** PRD §7.2 помечает происхождение Vosk как
   «отеч. (уточнить, A6)». В макете такая карточка получает подпись «происхождение уточняется»
   и **не попадает** в фильтр отечественных — от решения зависит состав каталога в профиле
   `domestic_only=true`.
2. **«Проверить обновления» в трее** при выключенных тумблерах остаётся **неактивным** (PRD F12),
   хотя кнопка «Проверить сейчас» в разделе «Сеть» работает всегда. В макете у неактивного пункта
   есть подсказка «Проверки выключены — включить в настройках». Оставляем расхождение или
   разрешаем ручную проверку и из трея?
3. **Новая настройка «Отсекать тишину в начале и конце (VAD)»** в «Продвинутых» — в PRD её нет
   (silero-vad упомянут только в NOTICE). Подтвердить или убрать.
4. **Минимальный размер окна.** 900×620 — рабочий размер; ниже 800×600 карточка модели ломается.
   Зафиксировать `minimumWidth/Height` в дизайн-спеке.
