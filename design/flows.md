...
> Стадия: **G2 пройден** — выбрано направление A «Панель»; развёрнуто в полный макет
> `design/mockups/final/` (24 экрана × 2 темы). Автор: `ux-analyst`, 2026-09-07,
> правки после G2 — 2026-09-08.
...
> и полный макет принятого направления A в `design/mockups/final/` (см. `final/index.html`).
...
flowchart TD
  ...
  NET -- да --> DL[Скачать 231,9 МБ → % → sha256 → Установлена]
...
flowchart LD
  ...
  PROC --> RES{результат}
  RES -- текст --> PASTE[буфер → Ctrl+V в окно, активное ДО пилюли → «Готово» 500 мс]
...
- **S15 Диск.** Перед скачиванием проверка `размер × 1,2`; не хватает → «Недостаточно места:
  нужно ~278 МБ, свободно 100 МБ» + «Освободить место» (открыть каталог) / «Выбрать модель полегче».
...
| `loading-model` | круговой индикатор | **Загружаю модель…** | до ответа воркера, ≤ 10 с (дедлайн загрузки; решение 2026-09-14) |
...
| `limit` | **иконка часов** (столбиков нет: с ними состояние неотличимо от «слушаю») | **Достигнут лимит записи** | 2 с, затем распознавание |
| `processing` | пульсирующие точки, жёлтый `#E8A33A` | **Распознаю…** | до результата |
| `done` | галочка, зелёный `#2FA36B` | **Готово** | **500 мс** (решение G2, 2026-09-08; PRD F3.2 уже приведён к 500 мс) |
...
Геометрия: высота 36, r = 18; **ширина резиновая под текст — 172 → 320 px** (решение G2, 2026-09-08):
**172 px — минимум для ВСЕХ состояний**, содержимое центрируется (сверка спеки с рендером 2026-09-08:
без пола «Готово» = 83 px, «Отменено» = 105 px, «Распознаю…» = 128 px, и окно дёргалось бы
в последовательности слушаю → распознаю → готово). Дальше ширина растёт по тексту,
обрезки многоточием быть не должно. Фактические ширины: загружаю 172 · слушаю 188 · молчит 273 ·
лимит 196 · распознаю 172 · готово 172 · пусто 176 · буфер 174 · отменено 172 · ошибка 214. Фиксированные 172 px Handy рассчитаны
на английские подписи и русским не годятся.
...
«Проверить обновления» (**активен всегда** — ручная проверка = явное действие пользователя, решение
2026-09-08; неактивен **только** в офлайн-режиме и при запрете политикой; PRD F12 требует правки) ·
«О программе» ·
«Выход» `Ctrl+Q`.
...
|---|---|---|
| `not-downloaded` | все поля, цветные полоски «Точность» / «Скорость» по шкале (spec §5.3), «419 МБ в памяти» | **Скачать** · Установить из файла · Подробнее |
...
| `installed` | «Установлена · 231,9 МБ на диске» | **Выбрать** · Удалить · Подробнее |
| `active` | рамка акцентом + бейдж «Установлена и активна»; замеренные здесь скорость и память — значение темнее и маркер ✓ («402 МБ в памяти ✓»), без слова «замерено» | Удалить **disabled** («Сначала выберите другую модель») |
...
(жёлтая полоса «Нет доступа к каталогу» + «Установить из файла…», при этом **встроенный список моделей
из пакета остаётся виден**: он не требует сети, у карточек неактивна только «Скачать» — уточнено
при разворачивании макета, 2026-09-08, ср. F15 «встроенный манифест — источник по умолчанию») ·
`empty-offline` («Офлайн-режим включён вами») ·
...
|---|---|
| **Вторая комбинация («нажать-нажать»)** | Строка показывается **только в режиме «нажать-нажать»**; в режиме «удерживать» она не нужна и скрыта (уточнение 2026-09-08, чтобы «Общие» помещались без прокрутки) |
| **Поле захвата хоткея** | `idle` (клавиша `Ctrl+Space` моно, кнопка «Изменить») · `capturing` («Нажмите комбинацию… Esc — отмена», рамка акцентом) · `conflict` (жёлтая подсказка «`Alt+F2` занята в системе: «Показать KRunner». Оставить? Комбинация может не сработать» · «Оставить» / «Выбрать другую») · `duplicate` («Эта комбинация уже назначена на toggle») · `not-grabbed` (красная «Комбинацию не удалось захватить — выберите другую») |
...
| Мало места | **Недостаточно места на диске** | Нужно ~278 МБ, свободно 100 МБ. Освободите место или выберите модель полегче (Vosk small — 26,7 МБ). | **Открыть папку моделей** · Выбрать другую |
| sha256 не совпал | **Не удалось загрузить модель** | Файл не прошёл проверку ни на одном сервере. Испорченный файл удалён, уже проверенные сохранены — попробуйте ещё раз позже. | **Повторить** |
...
- У каждой полоски «Точность»/«Скорость» — текстовое значение рядом (не только длина и цвет
  полоски); замер на этом компьютере отличается маркером ✓ и начертанием значения, а не только цветом.
...
4. **Полоска скорости до замера.** PRD требует показывать бенчмарк «пересчитанным коэффициентом
   машины», но коэффициент неизвестен до первого прогона. Трактовка: до первого замера — цифра
   авторов без пересчёта; после первой диктовки — замер с маркером ✓. **Изменено 2026-09-23:**
   полоски цветные по шкале (spec §5.3), подписи об источнике цифр на экране нет.
5. **«Готово» ≤ 300 мс vs. `done` 0,8 с в трее.** ✅ Закрыто на G2 (2026-09-08): пилюля держит
   «Готово» **500 мс**, трей — 0,8 с. PRD F3.2 обновлён (коммит 8a0bcb9).
...
## 8. Приёмка макетов: что решено на ⛔ G2 (2026-09-08)

1. ✅ **Направление — A «Панель»** (окно 900×620, сайдбар из 6 разделов + скрытая «Отладка»).
2. ✅ Пилюля — **резиновая под текст**, 172 → 320 px, многоточия не допускается.
3. ✅ Каталог показывается **целиком с прокруткой**, без «показать ещё»; фильтры «Все языки»
   и «Только отечественные».
4. ✅ «Готово» в пилюле — **500 мс**.

## 8.1 Полный макет: что решено 2026-09-08 и что осталось открытым (⛔ G3)

1. ✅ **Vosk и фильтр «Только отечественные»** — решено 2026-09-08 по итогам факт-чека
   (`research/catalog-numbers.md` §3): «отечественная» определяется **по юрлицу правообладателя**.
   Alpha Cephei Inc. — США (Делавэр), команда из России → подпись карточки
   «зарубежная · команда из России», в фильтр не попадает; в профиле `domestic_only=true`
   остаются 6 моделей (пять GigaAM + T-one).
2. ✅ **«Проверить обновления» в трее** — решено 2026-09-08: пункт **активен всегда**, ручная
   проверка равна явному действию пользователя. Неактивен только в офлайн-режиме и при запрете
   политикой. Требуется правка PRD F12.
3. ✅ **Тумблер VAD** — решено 2026-09-08: в v1 его **нет**, отсечение тишины работает автоматически;
   в «Продвинутых» остаётся только настраиваемый порог тишины −60 дБ (F6.2).
4. ✅ **Размер окна** — решено 2026-09-08: **900×620 фиксируется как `minimumWidth/Height`**;
   значение уходит в дизайн-спеку, в PRD F11 размера окна нет — добавить.
5. ✅ **Цифры каталога** — закрыто 2026-09-08: все 12 позиций взяты из
   `research/catalog-numbers.md` (размер = сумма точных байт файлов рантайма по HF API,
   десятичные МБ; WER — Russian LibriSpeech, бенчмарк onnx-asr; скорость — столбец «x64 RTFx int8»).
   Заглушек в макете нет. Правки задели **9 из 12 размеров** и **10 из 12 значений WER** —
   нужна ревизия PRD §7.2.
   - ~~Оговорка в подсказке про fp32-веса~~ и ~~«Бенчмарк fp32» на полоске скорости~~ —
     **сняты 2026-09-23** (тексты простым языком; на экране нет ни подписи источника, ни ссылки
     «Как мы считаем»). Где скорости в каталоге нет — «нет данных».
   - **«Нет данных по протоколу»** — Whisper small: полоски пунктиром, в карточке пояснение
     «FLEURS 11,4 % — другой набор; замерим на вашем компьютере». Свой замер — v0.2 (US-5.4).
   - **Лицензии Whisper раздельно:** large-v3-turbo — MIT, small и base — Apache-2.0.
