...
/* фильтр списка «Модели» = два чипа-сегмента, как в реализации (qml/components/FilterChip.qml, M6):
   высота 25 (текст + паддинг 4 сверху и снизу), паддинг 11 от края вместе с границей 1, радиус 14,
   зазор 6 (chipGap). Активный — primary / primary-fg / 500, второй — bg-surface / border / fg-secondary */
.fchips{display:flex;gap:6px;align-items:center;margin:0 0 12px}
.chip.seg{height:25px;box-sizing:border-box;padding:0 10px;line-height:23px;gap:0;white-space:nowrap}
...
/* подпись 11 px: при 11.5 «Скорость» шире 46 (51,6 в Chromium) и вылезала под полоску. Трекинг −0.02em —
   компенсация Chromium («Скорость» 47,6 → 45,8), чтобы край подписи стоял на 46, как в реализации */
.mm .mml{width:46px;margin-right:2px;text-align:right;font-size:11px;letter-spacing:-0.02em;white-space:nowrap;
  color:var(--fg3);flex:none}
...
/* нижняя строка начинается по левому краю имени и описания: отметка 18 + зазор 12 = 30 от внутреннего
   края карточки (решение заказчика 2026-09-26); правый край кнопок не меняется */
.mfoot{display:flex;align-items:flex-start;gap:12px;margin-top:6px;padding-left:30px}
...
/* --- пилюля: минимум ширины для ВСЕХ состояний (сверка спеки с рендером 2026-09-08) ---
   короткие подписи («Готово» 83 px, «Отменено» 105 px) заставляли окно дёргаться
   в последовательности слушаю → распознаю → готово. Теперь 172 px — пол для всех. */
.pill{min-width:172px;max-width:320px;justify-content:center}
