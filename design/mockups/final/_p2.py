...
def advanced(theme):
    ...
    g2 = group("Запись", [
        ...
        row("Порог тишины", slider(38, "−60 дБ"),
            ...
            "Отсечение тишины по краям записи работает само, отдельной настройки нет"),
...
def about(theme):
    ...
    return page(f"A · О программе — {th(theme)}",
                ...
                b + sech("Диалоги")
                + grid([stcell("NOTICE — лицензии компонентов и моделей", notice, "notice")], 1)
                + grid([stcell("PRIVACY — что уходит в сеть", privacy, "privacy")], 1)
                + sech("Статистика и удаление")
                + grid([stcell("статистика есть", stat_data, "data")], 1)
                + grid([stcell("статистики ещё нет", stat_empty, "empty")], 1)
