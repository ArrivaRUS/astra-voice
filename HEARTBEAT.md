...
- **Пробел M4 — GUI не загружал модель**: мостик написан (`core/model_request.py`, `model.load` по `hello` воркера, пилюля `loading-model` до ответа ≤ 10 с, changelog `0.1.0~m4.1`); **ревью Claude: B1 (`settings.to_dict()` вместо `extra`) и M1 (загрузка при самоперезапуске супервизора) — в цикле правок**; затем коммит C, пересборка m4.1 → `~/Desktop` (там сейчас сборка ДО правок B1/M1 — заменить). Апгрейд `onnxruntime` 1.25.1 опробован в worktree (ветка `wip/ort-1.25.1`, не влито): гейты зелёные, текст верный, НО onnx-asr 0.12.0 исключает 1.25.\* без причины и инференс ×2 медленнее на машине заказчика (p95 528→1141 мс под нагрузкой) — решение отложено: ресёрч причины исключения/1.27, чистый перезамер на свободной машине, альтернатива — seccomp-запрет AMX в воркере на 1.24.4 (`research/tech-amx-int8.md`, дополнение).
