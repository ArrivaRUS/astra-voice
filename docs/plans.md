...
| M10 | Корпоративный контур, трек B, Fly-адаптеры (S17), US-2.10 «Команда помощнику» по контракту K0 (1.3) | R2, K0+S9 (v0.2), C0/C1–C6 в Cowork, **У44 закрыт до F17** (T1 26.09) | v1.0 | [ ] |
...
## M6. Каталог `[~]` — v0.2 (состояние 23.09: блоки 1–5 + Р5 + Ц6 в main; открыты: полный загрузчик (очередь/пауза), US-11.3 OOM + состояние 20 (`wip/m6-oom`, ревью), карточка A «Три строки» + 20 состояний + снимки (`wip/m6-card`), полный загрузчик, benchmark; Popup «Как мы считаем» снят решением заказчика 23.09)
...
- [ ] Режим «только corp» (`corp_only`) для загрузки без публичного fallback; в v0.1 такого ключа нет (У97, T-123).
