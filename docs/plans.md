...
- [~] **Живой прогон Ц2 у заказчика (KDE, 2026-09-14 20:01, `0.1.0~m4.3`)**: 50/50 ok, 0 пусто, 0 ошибок микрофона; t_ms p50 269 · **p95 523 > 500** · max 777 — **не уложились на 23 мс**; хвост бимодальный с шагом ~250 мс (тик опроса воркера?) → debugger → фикс → повтор. Пробы m4.1/m4.2 вскрыли раскладку пакета (урок 008) и контракт IPC (урок 007 доп.); e2e-скрипты починены (`--delay 100`, `set-default-source`). Доказательства `spikes/m4_live/`. T-50/T-56/T-58, пробник `Ctrl+Alt+Space` — не сделаны.
