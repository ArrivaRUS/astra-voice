...
- [x] Корректное завершение (US-8.4): «Выход»/SIGTERM → восстановление буфера, `SIGTERM` воркеру, освобождение микрофона. **2026-09-11: `DictationRuntime.shutdown()` (отмена таймеров → снятие grab-ов → `audio.close` → `supervisor.stop` → трей → X11), вызывается из `finally` в `main()`. Явного восстановления буфера при выходе ПОСРЕДИ вставки нет: публичной ручки в `platform/paste.py` не существует — нужна зоне A.**
- [x] Сторож поля выбора комбинации (У47): `core/capture_watchdog.py` — отдельный `X11Display`, опрос ≤ 1 с, по сроку снимает захват, а при отказе `ungrab` закрывает сокет. **2026-09-11: покрыт unit; подключение к полю захвата — когда появится экран настроек (M5).**
